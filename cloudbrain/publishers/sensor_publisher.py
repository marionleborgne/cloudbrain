...
def _get_connector_class(device_type, mock_data_enabled):
  """
  Get the right Connector class implementation.
  
  :param device_type: (string) type of the device. 
  :param mock_data_enabled: (Boolean) 1 if mock data is enabled. 0 otherwise.
  :return conn: (cloudbrain.connectors.Connector) connector class. E.g:
   MuseConnector ,OpenBCIConnector, MockConnector.  
  """
  if device_type == "muse" and not mock_data_enabled:
    from cloudbrain.connectors.MuseConnector import MuseConnector as conn
  elif device_type == "openbci" and not mock_data_enabled:
    from cloudbrain.connectors.OpenBCIConnector import OpenBCIConnector as conn
  elif mock_data_enabled:
    from cloudbrain.connectors.MockConnector import MockConnector as conn
  else:
    raise ValueError("Device type '%s' not supported. "
                     "Supported devices are:%s" % (device_type,
                                                   _SUPPORTED_DEVICES))
  return conn
...
def get_args_parser():
  ...
  parser.add_argument('-i', '--device_id', required=True,
                      help="A unique ID to identify the device you are "
                           "sending data from. For example: 'octopicorn2015'")
  ...
  parser.add_argument('-n', '--device_name', required=True,
                      help="The name of the device your are sending data "
                           "from. Supported devices are: %s"
                           % _SUPPORTED_DEVICES)

  parser.add_argument('-c', '--cloudbrain', default=RABBITMQ_ADDRESS,
                      help="The address of the RabbitMQ instance you are "
                           "sending data to.\n Use %s to send data to our "
                           "hosted service. \n Otherwise use 'localhost' if "
                           "running CloudBrain locally" % RABBITMQ_ADDRESS)

  parser.add_argument('-o', '--output', default=None,
                      help="The named pipe you are sending data to "
                           "(e.g. /tmp/eeg_pipe)\n"
  ...
  parser.add_argument('-s', '--step_size', default=None,
                      help='Number of samples the chunk advances by '
                           '(default is equal to buffer size) ')

  parser.add_argument('-p', '--device_port',
                      help="Port used to get data from wearable device.\n"
                           "Common port values:\n"
                           "* 9090 for the Muse\n"
                           "* /dev/tty.usbserial-XXXXXXXX for the OpenBCI")

  parser.add_argument('-M', '--device_mac', help="MAC address of device used "
                                                 "for Muse connector.")
...
def run(device_name="muse",
        ...
        publisher_type="pika",
        device_mac=None):
  Connector = _get_connector_class(device_name, mock_data_enabled)

  if device_name == "muse" and not mock_data_enabled and not device_port:
    device_port = 9090
  ...
  if publisher_type == 'pika':
    print ("SUCCESS: device '%s' connected with ID '%s'\n Sending data to "
           "cloudbrain at address '%s' ..." % (device_name, device_id,
                                               cloudbrain_address))
  connector.start()
