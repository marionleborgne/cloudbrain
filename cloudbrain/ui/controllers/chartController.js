  ...
  angular.module('cloudbrain').controller('chartController', [
    ...
    function ($scope, $http, $interval, $log) {
      ...
      $scope.getData = function (device) {
        ...
        var metric = 'eeg';
        $scope.lastTimestamp = Math.floor(Date.now()/1000);
        $scope.cloudbrain = 'http://datastore.cloudbrain.rocks/data?device_name='+device.name+'&metric='+metric+'&device_id='+device.id+'&callback=JSON_CALLBACK&start='+$scope.lastTimestamp;
        ...
        for (var obj in $scope.data){
              ...
              for (var prop in $scope.data[obj]){
                if (prop != 'timestamp'){
                  ...
                  //$log.log(count);
                  //$scope.StockChart3.series[count].addPoint($scope.data[obj][prop], true, true);
                    $scope.chartStock.series[count].data.push($scope.data[obj][prop]);
                    $scope.chartStock.series[count].data = $scope.chartStock.series[count].data.slice(-150);
