...
# 
# # TODO: old code. Might be stale and/or unused and could use a good cleanup.
# import json
# with open("../../conf/supported_devices.json", "rb") as f:
#   SUPPORTED_DEVICES=json.load(f)
# 
# 
...
#   """
#   metadata = [metadata for metadata in SUPPORTED_DEVICES if metadata['device_name'] == device_name]
# 
#   if len(metadata) > 0:
...
#   """
#   metadata = [metadata for metadata in SUPPORTED_DEVICES if metadata['device_name'] == device_type]
# 
#   if len(metadata) > 0:
#     metrics = metadata[0]['metrics']
#   else:
...
#   return [device['device_name'] for device in SUPPORTED_DEVICES]
# 
# 
